...
public class BLStats {
	...
	public static void validateNotNull(double [] a) {
		if(a==null) {
			throw new IllegalArgumentException("\nno array input/ array does not exist\n");
		}
	}
     ...
     * 
     * @param a the array
     * @return the mean of the specified array {@code a[]};
     * 			{@code Double.NaN} if no such value
     */
    public static double median(double[] a) {
    	...
    	if(a.length%2==0) {
    		med = (a[(a.length-1)/2]+a[(a.length+1)/2])/2;	
    	}
    	else {
    		med = a[a.length/2];
    	}
